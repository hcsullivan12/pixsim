...
evgen::ActiveVolumeVertexSampler::ActiveVolumeVertexSampler(
  ...
  rndm::NuRandomService& rand_service, const geo::DetectorGeometryService& geom,
...
TLorentzVector evgen::ActiveVolumeVertexSampler::sample_vertex_pos(
  const geo::DetectorGeometryService& geom)
{
    ...
    while ( !ok && num_iterations < MAX_BOX_ITERATIONS ) {
      ...
      if ( fCheckActive ) {
        ...
        for (size_t iTPC = 0; iTPC < num_tpcs; ++iTPC) {
          ...
          double minX = 0;//tpc.MinX();
          double maxX = geom.DriftLength();//tpc.MaxX();
          double minY = -1*geom.DetHalfHeight();//tpc.MinY();
          double maxY = geom.DetHalfHeight();//tpc.MaxY();
          double minZ = 0;//tpc.MinZ();
          double maxZ = geom.DetLength();//tpc.MaxZ();
